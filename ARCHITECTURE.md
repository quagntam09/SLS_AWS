...
- `server/app/application/`: chứa use case và service làm việc với AWS, bao gồm tạo job, cập nhật trạng thái và đọc kết quả.
...
- **Package root**: code được tổ chức dưới `NSGA2IS-SLS`, nên `PYTHONPATH` của API, worker và wrapper phải trỏ đúng thư mục này để import `server.app.*` hoạt động nhất quán.
...
Các biến `APP_*` điều khiển hành vi tối ưu và được đọc bởi `server/app/config.py`, đặc biệt là:
...
- `server/app/application/` điều phối AWS và use case.
