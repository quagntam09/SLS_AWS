...
- Worker có thể chạy trên EC2 long-poll SQS hoặc trên Fargate thông qua EventBridge Pipes.
...
- `server/app/worker.py`: entrypoint worker event-driven cho EC2/Fargate.
...
4. Worker EC2 hoặc Fargate task nhận payload, chuyển job sang `RUNNING`.
...
- Worker: EC2 long-poll SQS hoặc Fargate task được kích hoạt bởi EventBridge Pipes.
